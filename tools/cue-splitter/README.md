...
Herramienta independiente para convertir una imagen FLAC acompañada por un
archivo `.cue` en pistas FLAC etiquetadas. No forma parte del backend ni del
flujo de reproducción de Hirmos.

## Requisitos

- Node.js 24;
- `ffmpeg` y `ffprobe` disponibles en `PATH`;
- un directorio con exactamente un `.cue` y el FLAC que este declara.

La versión inicial admite una única imagen FLAC por `.cue`. Decodifica y vuelve
a codificar en FLAC, por lo que conserva las muestras y la calidad originales.
No convierte fuentes con pérdida.
...
antes de tomar el nombre final. Una ejecución exitosa crea
`.hirmos-cue-split.json` con la huella de la fuente, del `.cue` y los límites de
cada pista. Como último paso crea o amplía `.ndignore` con una regla exacta para
el FLAC fuente: Navidrome ignora la imagen, pero puede indexar las pistas
separadas. Repetir `--apply` también repara esa regla si falta, sin regenerar el
audio.
...
como exclusión del directorio completo. La imagen FLAC y el `.cue` originales
no se eliminan ni se mueven.
